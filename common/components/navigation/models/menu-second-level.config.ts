import {
  INavigationMenuThirdLevelConfig,
  NavigationMenuThirdLevelConfig,
} from './menu-third-level.config';
...
export interface INavigationMenuSecondLevelConfig
...
export class NavigationMenuSecondLevelConfig extends NavigationBaseMenuLevelConfig {
  ...
  public constructor(config: INavigationMenuSecondLevelConfig) {
    ...
    this.nextLevels = (config.nextLevels ?? []).map(
      (l) => new NavigationMenuThirdLevelConfig(l),
    );
  }
